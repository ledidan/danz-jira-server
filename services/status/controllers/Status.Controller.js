...
const { createValidation } = require("../validations/status.validation");
...
module.exports = {
  getAll: async (req, res) => {
    ...
    try {
      ...
      let statusAll = await StatusModel.find().sort({ statusId: 1 });
      ...
      logInfo(`[STATUS] >> [GET ALL] response ${JSON.stringify(response)}`);
  ...
  create: async (req, res) => {
    ...
    try {
      logInfo(`[STATUS] >> [CREATE] payload ${JSON.stringify(req.body)}`);
      ...
      if (statusExist) {
        logInfo("[ERROR STATUS] [CREATE] Status name already exists!");
      ...
      logInfo(`[STATUS] >> [CREATE] response ${JSON.stringify(response)}`);
      return res.status(response.statusCode).send(response);
    } catch (err) {
