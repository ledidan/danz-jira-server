...
module.exports = {
  getAll: async (req, res) => {
    ...
    try {
      ...
      logInfo(`[PROJECT] >> [GET ALL] response ${JSON.stringify(response)}`);
  ...
  getProjectDetail: async (req, res) => {
    ...
    try {
      ...
      if (projectId === null) {
        logInfo("[ERROR PROJECT] [GET PROJECT DETAIL] Project not found!");
  ...
  createProjectAuthorize: async (req, res) => {
    ...
    try {
      ...
      if (!category) {
        logInfo("[ERROR PROJECT] [CREATE] Project category not found!");
  ...
  updateProject: async (req, res) => {
    ...
    const { projectName, creator, description, categoryId } = req.body;
    const response = {
      statusCode: 400,
      message: "Request failed",
      content: null,
    ...
    try {
      ...
      if (
        project.creator.id === _.toNumber(req.user.aud) &&
        _.get(user, "userId", false)
      ) {
      ...
      if (!_.isObject(updateProject) || !_.get(updateProject, "id", false)) {
        logInfo("[ERROR PROJECT] [EDIT] Edit project failed!");
  ...
  assignUserProject: async (req, res) => {
    ...
    try {
      logInfo(
        `[PROJECT] >> [ASSIGN USER PROJECT] payload ${JSON.stringify(req.body)}`
      );
      const { error } = assignUserProjectValidation(req.body);
      if (error) {
        logInfo(
          `[ERROR PROJECT] [ASSIGN USER PROJECT] ${JSON.stringify(error)}`
        );
        response.message = error.details[0].message;
        return res.status(response.statusCode).send(response);
      }
      ...
      if (_.includes(arrMemberIdInProject, user.userId)) {
        logInfo(
          "[ERROR PROJECT] [ASSIGN USER PROJECT] User already exists in the project!"
        );
        response.message = "User already exists in the project!";
        return res.status(response.statusCode).send(response);
      }

      const userAssignToProject = _.pick(user, [
        "userId",
        "name",
        "avatar",
        "email",
      ...
      const assignUserToProject = await ProjectModel.updateOne(
        { id: projectId },
        {
          $push: {
      ...
      if (!assignUserToProject) {
        logInfo(
          "[ERROR PROJECT] [ASSIGN USER PROJECT] Update failed, please try again"
        );
      ...
      logInfo(
        `[PROJECT] >> [ASSIGN USER PROJECT] response ${JSON.stringify(
          response
        )}`
      );
      return res.status(response.statusCode).send(response);
  ...
  removeUserFromProject: async (req, res) => {
    ...
    try {
      logInfo(
        `[PROJECT] >> [REMOVE USER PROJECT] payload ${JSON.stringify(req.body)}`
      );
      const { error } = assignUserProjectValidation(req.body);
      if (error) {
        logInfo(
          `[ERROR PROJECT] [REMOVE USER PROJECT] ${JSON.stringify(error)}`
        );
        response.message = error.details[0].message;
        return res.status(response.statusCode).send(response);
      }
      ...
      const arrMemberRemoved = project.members.filter(
        (mem) => mem.userId !== userId
      );

      const removeUserFromProject = await ProjectModel.updateOne(
        { id: projectId },
        {
      ...
      if (!removeUserFromProject) {
        logInfo(
          "[ERROR PROJECT] [REMOVE USER PROJECT] Update failed, please try again"
        );
        response.message = "Update failed, please try again";
        return res.status(response.statusCode).send(response);
      }
      ...
      logInfo(
        `[PROJECT] >> [REMOVE USER PROJECT] response ${JSON.stringify(
          response
        )}`
      );
      return res.status(response.statusCode).send(response);
  ...
  createTask: async (req, res) => {
    ...
    try {
      logInfo(`[PROJECT] >> [CREATE TASK] payload ${JSON.stringify(req.body)}`);
      // LETS VALIDATE THE DATA BEFORE WE A USER
  ...
  updateTask: async (req, res) => {
    ...
    try {
      logInfo(`[PROJECT] >> [UPDATE TASK] payload ${JSON.stringify(req.body)}`);
      // LETS VALIDATE THE DATA BEFORE WE A USER
  ...
  getTaskDetail: async (req, res) => {
    ...
    try {
      ...
      if (taskId === null) {
        logInfo("[ERROR PROJECT] [GET TASK DETAIL] Task not found!");
        response.message = "Task not found!";
        return res.status(response.statusCode).send(response);
      }

      const getListCommentByTaskId = await CommentModel.find({ taskId }).sort({
        createdAt: -1,
      });
  ...
  removeTask: async (req, res) => {
    ...
    try {
      ...
      if (taskId === null) {
        logInfo("[ERROR PROJECT] [REMOVE TASK] Task not found!");
        response.message = "Task not found!";
        return res.status(response.statusCode).send(response);
      }
  ...
  assignUserTask: async (req, res) => {
    ...
    try {
      logInfo(
        `[PROJECT] >> [ASSIGN USER TASK] payload ${JSON.stringify(req.body)}`
      );
      ...
      if (_.includes(arrAssignessIdInTask, userId)) {
        logInfo(
          "[ERROR PROJECT] [ASSIGN USER TASK] User already assign in the task!"
        );
      ...
      const assignUserToTask = await TaskModel.updateOne(
        { taskId },
        {
          $push: {
      ...
      if (!assignUserToTask) {
        logInfo(
          "[ERROR PROJECT] [ASSIGN USER TASK] Update failed, please try again"
        );
        response.message = "Update failed, please try again";
        return res.status(response.statusCode).send(response);
      }
      ...
      logInfo(
        `[PROJECT] >> [ASSIGN USER TASK] response ${JSON.stringify(response)}`
      );
      return res.status(response.statusCode).send(response);
    } catch (err) {
  ...
  removeUserFromTask: async (req, res) => {
    ...
    try {
      logInfo(
        `[PROJECT] >> [REMOVE USER TASK] payload ${JSON.stringify(req.body)}`
      );
      ...
      const arrMemberRemoved = task.assigness.filter(
        (mem) => mem.id !== userId
      );

      const removeUserFromTask = await TaskModel.updateOne(
        { taskId },
        {
      ...
      logInfo(
        `[PROJECT] >> [REMOVE USER TASK] response ${JSON.stringify(response)}`
      );
      return res.status(response.statusCode).send(response);
    } catch (err) {
  ...
  updateDescription: async (req, res) => {
    ...
    try {
      ...
      if (error) {
        logInfo(
          `[ERROR PROJECT] [UPDATE DESCRIPTION] ${JSON.stringify(error)}`
        );
        response.message = error.details[0].message;
        return res.status(response.statusCode).send(response);
      }
  ...
  updateTimeTracking: async (req, res) => {
    ...
    try {
      logInfo(
        `[PROJECT] >> [UPDATE TIME TRACKING] payload ${JSON.stringify(
          req.body
        )}`
      );
      // LETS VALIDATE THE DATA
      ...
      if (error) {
        logInfo(
          `[ERROR PROJECT] [UPDATE TIME TRACKING] ${JSON.stringify(error)}`
        );
        response.message = error.details[0].message;
        return res.status(response.statusCode).send(response);
      }
      ...
      response.message = "Update task successfully!";
      logInfo(
        `[PROJECT] >> [UPDATE TIME TRACKING] response ${JSON.stringify(
          response
        )}`
      );
      return res.status(response.statusCode).send(response);
