...
module.exports = {
  ...
  editUser: async (req, res) => {
    ...
    try {
      ...
      if (!userExist) {
        logInfo("[ERROR USER] [EDIT USER] Account info not found");
      ...
      if (!updated) {
        logInfo(
          "[ERROR USER] [EDIT USER] Update account info failed, please try again"
        );
  ...
  deleteUser: async (req, res) => {
    ...
    try {
      logInfo(`[USER] >> [DELETE USER] params ${JSON.stringify(req.query)}`);
      ...
      if (id !== req.user.aud) {
        ...
        return res
          .status(403)
          .send("The account is different from the one logged in");
      }
      ...
      logInfo(`[USER] >> [DELETE USER] response ${JSON.stringify(response)}`);
      return res.status(response.statusCode).send(response);
    } catch (err) {
  ...
  getUser: async (req, res) => {
    ...
    try {
      ...
      logInfo(`[USER] >> [GET USER] response ${JSON.stringify(response)}`);
      return res.status(response.statusCode).send(response);
    } catch (err) {
